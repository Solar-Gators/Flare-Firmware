...
/**
 * @brief Generates the required bit to solve for even parity.
 * @param byte The given byte to check parity.
 * @return Returns 1 if the given byte is odd or 0 if it is even.
 */
...
bool generateBalCmd(uint8_t cmdArray[2], uint8_t const CRC4LUT[256], uint8_t chargeBM, uint8_t dischargeSyncBM, uint8_t dischargeNonBM= 0);
    ...
    public:
        ...
        */
        ...
        /** @brief Fetches the written balance command from the LTC.
         *  @return Returns the pointer to the first of 2 bytes of the balance command.
         */
        uint8_t* readbackBalCmd();

        /**@brief Fetches the balance status from the LTC.
         * @return Returns the pointer to the first of 2 bytes of the balance command.
         */
        uint8_t* readBalanceStatus();

        /**@brief Begins balancing based on the last latched balance command.
         */
        void executeBalanceCommand();

        /**@brief Begins balancing based on the last latched balance command.
 */
        void haltBalanceCommand();
        ...
        /** @brief Getter function for the device address.
         *  @return Returns the uint8_t of the device address.
         */
        uint8_t readAddress();
