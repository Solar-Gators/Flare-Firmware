...
{
    CANDevice::getInstance().hcan = hcan;

#if defined(HAL_CAN_MODULE_ENABLED)
    // ===================== bxCAN =====================
    if (CANDevice::getInstance().filters_.empty())
    {
        CanFilter_t filter = {};
    ...
    {
        for (const auto &filter : CANDevice::getInstance().filters_)
        {
            TRY(HAL_CAN_ConfigFilter(hcan, &filter));
        }
    }
...
HAL_StatusTypeDef CANDevice::AddFilterRange(
    uint32_t can_id, uint32_t range, uint32_t id_type, uint32_t rtr_mode, uint32_t priority)
{
    std::vector<CanFilter_t> *filters_ = &(CANDevice::getInstance().filters_);
