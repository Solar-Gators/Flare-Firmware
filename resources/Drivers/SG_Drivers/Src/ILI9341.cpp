...
static const uint8_t initcmd[] = {
    ...
    ILI9341_MADCTL,
    1,
    ILI9341_MADCTL_MX | ILI9341_MADCTL_BGR,  // Memory Access Control
    ILI9341_PIXFMT,
    1,
    0x55,
    ILI9341_FRMCTR1,
...
#if UI_USE_HAL
    HAL_GPIO_WritePin(
        LCD_DATA0_GPIO_Port_, LCD_DATA0_Pin_, static_cast<GPIO_PinState>(data & 0x01U));
    HAL_GPIO_WritePin(
        LCD_DATA1_GPIO_Port_, LCD_DATA1_Pin_, static_cast<GPIO_PinState>(data & 0x02U));
    HAL_GPIO_WritePin(
        LCD_DATA2_GPIO_Port_, LCD_DATA2_Pin_, static_cast<GPIO_PinState>(data & 0x04U));
    HAL_GPIO_WritePin(
        LCD_DATA3_GPIO_Port_, LCD_DATA3_Pin_, static_cast<GPIO_PinState>(data & 0x08U));
    HAL_GPIO_WritePin(
        LCD_DATA4_GPIO_Port_, LCD_DATA4_Pin_, static_cast<GPIO_PinState>(data & 0x10U));
    HAL_GPIO_WritePin(
        LCD_DATA5_GPIO_Port_, LCD_DATA5_Pin_, static_cast<GPIO_PinState>(data & 0x20U));
    HAL_GPIO_WritePin(
        LCD_DATA6_GPIO_Port_, LCD_DATA6_Pin_, static_cast<GPIO_PinState>(data & 0x40U));
    HAL_GPIO_WritePin(
        LCD_DATA7_GPIO_Port_, LCD_DATA7_Pin_, static_cast<GPIO_PinState>(data & 0x80U));
    // Pulse Write
    HAL_GPIO_WritePin(LCD_WRITE_GPIO_Port_, LCD_WRITE_Pin_, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(LCD_WRITE_GPIO_Port_, LCD_WRITE_Pin_, GPIO_PIN_SET);
#else
    // Setup data
    LCD_DATA0_GPIO_Port_->ODR ^=
        (-((data & 0x01U) >> 0) ^ LCD_DATA0_GPIO_Port_->ODR) & (1 << LCD_DATA0_Pos_);
    LCD_DATA1_GPIO_Port_->ODR ^=
        (-((data & 0x02U) >> 1) ^ LCD_DATA1_GPIO_Port_->ODR) & (1 << LCD_DATA1_Pos_);
    LCD_DATA2_GPIO_Port_->ODR ^=
        (-((data & 0x04U) >> 2) ^ LCD_DATA2_GPIO_Port_->ODR) & (1 << LCD_DATA2_Pos_);
    LCD_DATA3_GPIO_Port_->ODR ^=
        (-((data & 0x08U) >> 3) ^ LCD_DATA3_GPIO_Port_->ODR) & (1 << LCD_DATA3_Pos_);
    LCD_DATA4_GPIO_Port_->ODR ^=
        (-((data & 0x10U) >> 4) ^ LCD_DATA4_GPIO_Port_->ODR) & (1 << LCD_DATA4_Pos_);
    LCD_DATA5_GPIO_Port_->ODR ^=
        (-((data & 0x20U) >> 5) ^ LCD_DATA5_GPIO_Port_->ODR) & (1 << LCD_DATA5_Pos_);
    LCD_DATA6_GPIO_Port_->ODR ^=
        (-((data & 0x40U) >> 6) ^ LCD_DATA6_GPIO_Port_->ODR) & (1 << LCD_DATA6_Pos_);
    LCD_DATA7_GPIO_Port_->ODR ^=
        (-((data & 0x80U) >> 7) ^ LCD_DATA7_GPIO_Port_->ODR) & (1 << LCD_DATA7_Pos_);
    // Pulse Write
...
char ILI9341::SetWindow(uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye)
{
    ...
    if ((xs > xe) || (xe > width_) || (ys > ye) || (ye > height_))
    {
        // out of range
...
void ILI9341::DrawFastHLine(uint16_t x, uint16_t y, uint16_t length, uint16_t color)
{
    int16_t x2;
...
void ILI9341::DrawFastVLine(uint16_t x, uint16_t y, uint16_t length, uint16_t color)
{
    int16_t y2;
...
void ILI9341::DrawChar(int16_t x, int16_t y, unsigned char c, uint16_t color, uint8_t size)
{
    DrawChar(x, y, c, color, textbgcolor, size, size);
}

void ILI9341::DrawChar(int16_t x,
                       int16_t y,
                       unsigned char c,
                       uint16_t color,
                       uint16_t bg,
...
void ILI9341::FillRect(uint16_t x1, uint16_t y1, uint16_t w, uint16_t h, uint16_t color)
{
    ...
    // Initial off-screen clipping
    if ((w <= 0) || (h <= 0) || (x1 >= width_) || (y1 >= height_) || ((x2 = x1 + w - 1) < 0) ||
        ((y2 = y1 + h - 1) < 0))
        return;
    if (x1 < 0)
    {  // Clip left
